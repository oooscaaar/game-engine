...
unsigned long long GetTotalSystemPhysicalMemory()
{
	...
	int mem = 0;
	int n = sscanf_s(memTotal.c_str(), "%d", &mem);
	if (n == 1)
...
int GetMaxSimultaneousThreads()
{
	...
	int numCPUs = 0;
	int n = sscanf_s(r.c_str(), "%d", &numCPUs);
...
std::string GetOSDisplayString()
{
	...
	int n = sscanf_s(marketingVersion.c_str(), "%d.%d", &majorVer, &minorVer);
	if (n != 2)
		return uname;
...
int GetMaxSimultaneousThreads()
{
	...
	int n = sscanf_s(r.c_str(), "%d-%d", &nCoresMin, &nCoresMax);
	if (n == 2)
